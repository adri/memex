defmodule MemexWeb.SidebarsComponent do
  ...
  def render(assigns) do
    ~F"""
    <div phx-hook="Sidebar" id="sidebars" data-open={length(@sidebars) > 1} aria-hidden />
    <div
      :for.with_index={{hit, index} <- @sidebars}
      id={"sidebar-#{index}"}
      class={
        "fixed inset-0 transition backdrop-filter backdrop-blur-sm",
        "backdrop-filter-none pointer-events-none": hit["closed"],
        "bg-black bg-opacity-30": not hit["closed"]
      }
    >
      ...
      <div class={"fixed border-l dark:border-gray-700 dark:bg-gray-800 bg-gray-50 p-5 inset-y-0 right-0 shadow-2xl #{width(index)} overflow-scroll transform transition-transform #{if hit["closed"] do
        ...
        <div :if={hit["type"] === "person"}>{live_render(@socket, MemexWeb.Sidebars.PersonLive, id: "person", session: %{"hit" => hit})}</div>
        <div :if={hit["type"] === "activity"}>{live_render(@socket, MemexWeb.Sidebars.ActivityLive, id: "activity", session: hit)}</div>
        <div
